...
この記事は、Swiftでの開発を最近復帰した方々でも`Sendable`を理解するための助けとなるように、Swift6未満ではどのように対応していたのかがわかる記事となっています。
...
それらの説明はほかの記事を参考にしていただき、この記事ではそうした他の要素をなるべく省いたうえで、`Sendable`に焦点を当てて説明していきます。
...
具体的な説明は後ほど行うとして、まずはこれらを以前（Swift6未満）はどのような形でこれらに対応していたのかを説明していきます。

## Swift6未満のスレッドセーフの方法
並行処理のスレッドセーフを保証することは、アプリケーション開発にとって重要な部分です。
Swift6未満では、どのように対応していたかというと`DispatchQueue（GCD）`をもちいて対応していました。
...
```
* actorはデフォルトでスレッドセーフとなります
* awaitを使わないとactorのメソッドが呼び出せないため、必然的にスレッドセーフなコードとなります。
...
## おわり
以上が今回の記事となります。
ちょっとVisionOSやサードパーティライブラリを使うときに、少しハマってしまい、`Sendable`の知識が必要なところがありました。
